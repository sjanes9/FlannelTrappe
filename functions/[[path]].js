 ...
 *
 * Runs as a Cloudflare Pages Function on the same domain as the site,
 * so there is no CORS to configure and no separate Worker to deploy.
 *
 * Bindings required (set in the Pages project settings):
...
export async function onRequest({ request, env, params }) {
  ...
  if (route === "site") {
    ...
    if (method === "GET") {
      ...
      if (!stored) return json({ error: "nothing published yet" }, 404);
